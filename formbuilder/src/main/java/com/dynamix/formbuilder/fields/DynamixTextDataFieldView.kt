 ...
 */
class DynamixTextDataFieldView : DynamixBaseFieldView(), DynamixFormDataHandler {
    ...
    override fun render(field: DynamixFormField): View? {
        ...
        if (field.label.isNotEmpty() && !field.label.equals("", ignoreCase = true)) {
            val tvLabelData = TextView(ctx)
            tvLabelData.text = field.label
            tvLabelData.tag = field.tag + "__textdata"
            formFieldView.formField = field
            formFieldView.view = tvLabelData
